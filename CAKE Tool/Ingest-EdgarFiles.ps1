...
function ConvertTo-EDGARItem {
    ...
    for ($i = 0; $i -lt $Filings.accessionNumber.Count -and $i -lt 8; $i++) {
...
Write-Host "Total filings to download: $($sortedFilings.Count)"
$i=1
$req=0
...
# output the $def to a CSV for reference
$def | Export-Csv -Path './def_filings.csv' -NoTypeInformation
